...
fc = 0.02 * float(gxsm.get("dsp-SPMC-LCK-FREQ"))
...
def iir_alpha (norm_fc):  # f_norm = fs/f
	y = math.tan (math.pi*norm_fc)
	return 2*y/(1+y)
...
if use_iir >= 0:
	if use_iir > 0 and use_iir < 1:
		alpha = use_iir
	else:
		alpha = iir_alpha (fc_norm)
	sos =  np.array([[ alpha, 0.00 , 0.0,   1., alpha-1.0, 0.], [ 1., 0., 0.,        1., 0., 0.]])
	print ('IIR-sos for norma_cut= ', fc_norm, alpha,' : ', sos)
	print (sos[0, 0], sos[0, 1])
...
if 1:
	...
	if 2: # Int Calc
		...
		#plt.plot (vpdata['Time-Mon'], z0[0], alpha=0.4, linewidth=4, label='Z0S0-Q')
		#plt.plot (vpdata['Time-Mon'], z1[0], alpha=0.4, linewidth=4, label='Z1S0-Q')
		#plt.plot (vpdata['Time-Mon'], z0[1], alpha=0.4, linewidth=4, label='Z0S1-Q')
		#plt.plot (vpdata['Time-Mon'], z1[1], alpha=0.4, linewidth=4, label='Z1S1-Q')
	#plt.ylim (0.0, 1.)
	#plt.xlim (0.0, 5.)
	plt.legend()
	plt.grid()
