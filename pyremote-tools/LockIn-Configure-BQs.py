...
def ellipt_hi_filter(order=2, cutoff=0.2, sa=50, rp=0.5, filter_type='highpass', fs=1.0):
    nyquist = 0.5 * fs  # Nyquist frequency
    normal_cutoff = cutoff / nyquist
    sos = ellip(order, rp, sa, cutoff, btype=filter_type, output='sos')
    sos = np.atleast_2d(sos)
...
def notch_filter(w0=50.0, Q=100000.0, fs=2e6):
    sos = signal.iirnotch(w0, Q, fs=fs)
    sos = np.atleast_2d(sos)
    return sos
...
def plot_frequency_response(b, a, cutoff):
    ...
    plt.grid()
    plt.savefig('/tmp/filter_response.png')
    plt.show()
...
if 1:
	...
	s=0
	for i in range (0,6):
		gxsm.set("dsp-SPMC-ZS-BQ-COEF-BA0{}".format(i), '0.1234')
		gxsm.set("dsp-SPMC-ZS-BQ-COEF-BA0{}".format(i), '0')
		gxsm.set("dsp-SPMC-ZS-BQ-COEF-BA0{}".format(i), str(sos[s][i]))
