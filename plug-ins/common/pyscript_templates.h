...
def run_auto_approach(I=0.03):
	print ('Start Tip Auto Approach Procedure')
	print ('VZ = ', VZ_tip(), 'V')

	while not checkRange(I) and abs(dFreq()) < 5 and UserOK():
		print ('dFreq: ', abs(dFreq()), 'Hz   Z=', VZ_tip(), ' V')
		retract()
		thv.TipDownApp(2)
		time.sleep(0.5)
		if abs(dFreq()) > 5:
			...
			time.sleep(1.5)
		print (abs(dFreq()), 'Hz')
	print ('Finished or Safety Abort. dFreq: ', abs(dFreq()), 'Hz   Z=', VZ_tip(), ' V')
