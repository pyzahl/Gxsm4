...
\begin{alltt}
 Create using: $ python3 -m venv .gxsm4/pyaction/GxsmPythonVenv
\end{alltt}
...
static gboolean main_context_action_from_thread (gpointer user_data){
	...
	if (!PyArg_ParseTuple(idle_data->args, "s|s", &parameter, &value)){
                PI_DEBUG(DBG_L2, "pyremote Action ** idle cb: PyObjArgs invalid: gxsm.action('action_id',[optional string arg])");
                UNSET_WAIT_JOIN_MAIN;
                return G_SOURCE_REMOVE;
        }
...
static PyObject* remote_getslice_v(PyObject *self, PyObject *args)
{
	PI_DEBUG(DBG_L2, "pyremote: getslice_v");
	long ch,xl,t,yi,yn;

	//PyObject *obj;
        
	if (!PyArg_ParseTuple (args, "lllll", &ch, &xl, &t, &yi, &yn))
		return Py_BuildValue("i", -1);
...
static PyObject* remote_getprobe_event(PyObject *self, PyObject *args)
{
	...
	Scan *src =gapp->xsm->GetScanChannel (ch);
        ...
        if (src){
                ...
                g_message ("remote_getprobe_events data from mem2d scan data, looking for ProbeEvent #%d", nth);
                ...
                while (pel){
                        ...
                        if (sn == nth || (nth == -1 && !g_slist_next (pel))){
                                ...
                                if (pe->get_parent_scan_event ())
                                        pe->get_parent_scan_event ()->get_position (xy[0], xy[1]);
                                else
                                        g_message ("WARNING: can not get position");

                                src->World2Pixel (xy[0],xy[1], xy[2], xy[3]);
                                
                                g_message ("Creating PyArray for probe data event #%d: shape %d , %d @XY %g %g, ji: %g %g", sn, dims[0], dims[1], xy[0], xy[1], xy[2], xy[3]);
                                double *darr2 = (double*) malloc(sizeof(double) * dims[0]*dims[1]);
                                double *dp=darr2;
                                ...
                                PyTuple_SetItem(ret, 3, pyarrposxy );
...
static PyMethodDef GxsmPyMethods[] = {
	...
	{"get_slice_v", remote_getslice_v, METH_VARARGS, "Get Image Data Slice in V at x (Lines) from Scan in channel ch, yi ... yi+yn: [nv,ny,array]=gxsm.get_slice_v (ch, x, t, yi, yn)"},
	{"get_probe_event", remote_getprobe_event, METH_VARARGS, "Get Probe Event Data from Scan in channel ch, nth: [sets, labels, units, position[x,y,ix,iy]]=gxsm.get_probe_event (ch, nth)  ** if nth>existing, returns #probe events, nth=-99: removes all events "},
...
void py_gxsm_console::initialize(void)
{
	...
	if (!Py_IsInitialized()) {
		PI_DEBUG_GM (DBG_L1, "** Initializing Python interpreter, loading gxsm module and stdout redirection helper **");
                ...
                push_message_async (N_("\n\nChecking for Gxsm Python Venv. -- You can setup a Gxsm Python dedicated virtual environmnet if desired:\n to create run $python3 -m venv ~/.gxsm4/pyaction/GxsmPythonVenv"));
                // create using: $ python3 -m venv .gxsm4/pyaction/GxsmPythonVenv
                
                // Testing Venv use
                gchar* venv_path = g_strconcat (g_get_home_dir (), "/.gxsm4/pyaction/GxsmPythonVenv", NULL);
...
PyObject *py_gxsm_console::create_environment(const gchar *filename) {
        ...
        push_message_async ("Create Environment for:");
        push_message_async (filename);

        PI_DEBUG_GM (DBG_L2, "py_gxsm_console::create_environment ** DICT COPY");
...
static GActionEntry win_py_gxsm_action_entries[] = {
	...
	{ "pyremote-configure", py_gxsm_console::configure_callback, NULL, NULL, NULL },
        { "pyremote-restart-interpreter", py_gxsm_console::restart_interpreter_callback, NULL, NULL, NULL },
};
...
gpointer PySHMServer_Run(gpointer data){
        ...
        for (int sig;;){
                ...
                if (sigwait (&wait_set, &sig)) // waiting here for signal
                        g_warning ("PhSHM thread: sigwait error");
                if (sig != SIGUSR1){
                        g_warning ("PhSHM thread: unexpected signal received. Ignoring.");
                        continue;
                }
                ...
                for (PyMethodDef *ml=GxsmPyMethods; ml->ml_name; ml++){
                        ...
                        if (!strncmp (ml->ml_name, (char*)shm_data_ptr, 64)){ // first 64 bytes holding the method name
                                ...
                                break;
                        } else if (!(ml+1)->ml_name){ // at end?
                                *(gint64*)(shm_data_ptr+100) = -4; // RETURN code Error.
                                g_warning("PySHM: Unknown PyRemote Method or Action Request. Ignoring >gxsm.%s(..)<", (char*)shm_data_ptr);
                        }
                }
        }
