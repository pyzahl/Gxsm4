...
class StepResponse():
	...
	def setup(self, value=0.):
		...
		if self.mode == "Amp":
			# Ampl step
			s1 = self.parent.mk2rte.lookup_signal_by_name ("PLL Res Amp LP")
			s2 = self.parent.mk2rte.lookup_signal_by_name ("PLL Exci Amp LP")
			self.parent.mk2rte.change_signal_input (0, s1 , DSP_SIGNAL_SCOPE_SIGNAL1_INPUT_ID)
			self.parent.mk2rte.change_signal_input (0, s2 , DSP_SIGNAL_SCOPE_SIGNAL2_INPUT_ID)
			self.parent.mk2rte. set_PLL_amplitude_step (self.value)

		else:
			# Phase Step
			s1 = self.parent.mk2rte.lookup_signal_by_name ("PLL Exci Frq LP")
			s2 = self.parent.mk2rte.lookup_signal_by_name ("PLL Res Ph LP")
			self.parent.mk2rte.change_signal_input (0, s1 , DSP_SIGNAL_SCOPE_SIGNAL1_INPUT_ID)
			self.parent.mk2rte.change_signal_input (0, s2 , DSP_SIGNAL_SCOPE_SIGNAL2_INPUT_ID)
			self.parent.mk2rte. set_PLL_phase_step (self.value)

class TuneScope():

    def __init__(self, parent, fc=30760.0, span=100., Fres=0.1, int_ms = 100.):
	Xsignal = parent.mk2rte.lookup_signal_by_name("PLL Res Amp LP")
	Ysignal = parent.mk2rte.lookup_signal_by_name("PLL Res Ph LP")
...
#			print self.Fit
		...
		def update_tune(set_data, Xsignal, Ysignal):
			Filter64Out = parent.mk2rte.read_PLL_Filter64Out ()
...
#				print "Ph UnWrap neg=", cur_ph
			...
			if self.pos >= self.points:
				...
				self.run_button.set_label("RESTART")
                                ...
                                self.mode2f = 1
				parent.mk2rte.adjust_PLL_sine (self.volumeSine, self.fitresults["f0"], self.mode2f)
                                time.sleep (0.2)
				...
				parent.mk2rte.adjust_PLL_sine (self.volumeSine, self.FSineHz, self.mode2f)
...
class RecorderDeci():
	...
	if not parent.wins.has_key(name):
		...
		tr=0
                c=0
		#lab = gobject.new(gtk.Label, label="CH1: %s"%Xsignal[SIG_NAME] + ", Scale in %s/div"%Xsignal[SIG_UNIT])
		lab = gobject.new(gtk.Label, label="CH1")
		...
		lab = gobject.new(gtk.Label, label="CH2")
		table.attach(lab, c, c+1, tr, tr+1)
                tr=tr+1
		self.M2scale = gtk.Entry()
		self.M2scale.set_text("1.0")
		...
		table.attach(self.T1, c, c+1, tr, tr+1)
                ...
                self.lastevent = parent.mk2rte.read_recorder_deci (4096, "", True)
		...
		def update_recorder():
			...
			try:
				...
				m1th = 0.0
                        ...
                        rec = parent.mk2rte.read_recorder_deci (4096, self.logfile)
                        
                        scope.set_data (rec/256./32768.*10./m1scale_div, rec[-1024:-1]/dscale/m2scale_div)
                        ...
                        if m1th >= 0.0:
                                ...
                                if abs (ma) > m1th or abs (mi) > m1th:
                                        ...
                                        if self.logcount > 0:
                                                ...
                                                scope.set_data_with_uv (rec/256./32768.*10./m1scale_div, rec[-1024:-1]/dscale/m2scale_div, zeros(0), self.lastevent/256./32768.*10./m1scale_div, self.lastevent[-1024:-1]/dscale/m2scale_div)
                                        else:
                                                if self.logfile != '':
                                                        self.logfile = ''
                                                        scope.set_data_with_uv (rec/256./32768.*10./m1scale_div, rec[-1024:-1]/dscale/m2scale_div, zeros(0), zeros(0), zeros(0))
		...
		def toggle_run_recorder (b):
			if self.run:
				...
				gobject.timeout_add (100, update_recorder)
...
##                   "SR MK3 read D-FIFO128": readFIFO128,
	...
	win = gobject.new(gtk.Window,
		          type=gtk.WINDOW_TOPLEVEL,
		          title='SR MK2-Pro RTEngine Configurator '+version,
		          allow_grow=True,
		          allow_shrink=True,
		          border_width=5)
	...
	apps = self.app_buttons.keys()
	apps.sort()
	for appname in apps:
            print (appname)
	    button = gobject.new (gtk.Button, label=appname)

	    if self.app_buttons[appname]:
		button.connect ("clicked", self.app_buttons[appname])
	    else:
...
#                    print deltasRe, deltasIm
    ...
    def wave_generator (self, _button):
            ...
            if not self.wins.has_key(name):
                    ...
                    self.wv_ampl = 1.0
                    self.wvl_ampl = gobject.new(gtk.Label, label="Amplitude in V:")
                    ...
                    self.wv_pwd = 1.0
                    self.wvl_pwd = gobject.new(gtk.Label, label="Pulse Width in ms:")
                    ...
                    self.wvl_period = gobject.new(gtk.Label, label="Period in ms:")
                    ...
                    self.wvl_cycles = gobject.new(gtk.Label, label="# Cycles:")
    ...
    def make_signal_selector (self, _input_id, sig, prefix, flag_nullptr_ok=0):
		...
		item = self.make_menu_item("???", self.mk2rte.change_signal_input, sig, -1)
		menu.append(item)
		SIG_LIST = self.mk2rte.get_signal_lookup_list()
		for signal in SIG_LIST:
			if signal[SIG_ADR] > 0:
				item = self.make_menu_item(prefix+"S%02d:"%signal[SIG_INDEX]+signal[SIG_NAME], self.mk2rte.change_signal_input, signal, _input_id, self.global_vector_index)
				menu.append(item)
				if signal[SIG_ADR] == sig[SIG_ADR]:
					i_actual_signal=i
				i=i+1
		if flag_nullptr_ok:
			item = self.make_menu_item("DISABLED", self.mk2rte.disable_signal_input, 0, _input_id, self.global_vector_index)
			menu.append(item)
...
##			separator.show()
			...
			if flashdbg:
				...
				lab = gobject.new(gtk.Label(), label=self.mk2rte.flash_dump(0, 2))
				box1.pack_start(lab, expand=False)
				lab.show()
    ...
    def main(self):
	    gobject.timeout_add (timeout_DSP_status_reading, self.mk2rte.get_status)	
	    gobject.timeout_add (timeout_DSP_signal_lookup_reading, self.mk2rte.read_signal_lookup, 1)
