...
class HV5App(Gtk.Application):
        ...
        def on_button_toAFM(self, w):
                gxsm.set ('dsp-adv-dsp-zpos-ref', gxsm.get ('dsp-adv-dsp-zpos-mon'))
                gxsm.set_current_level_0 () ## normal Z control
                gxsm.set ('dsp-fbs-bias', 0.001)
                gxsm.set ('dsp-fbs-mx0-current-level', 0.1)
                
        def on_button_toSTM(self, w):
                gxsm.set ('dsp-adv-dsp-zpos-ref', 2+gxsm.get ('dsp-adv-dsp-zpos-mon'))
                time.sleep(0.2)
                gxsm.set_current_level_0 () ## normal Z control
                time.sleep(0.2)
        ...
        def do_activate(self):
                ...
                if not self.window:
                        ...
                        button = Gtk.Button(label="Z-Control Set")
                        button.connect("clicked", self.on_button_zcontrol)
                        grid_chv5c.attach(button, 17, 9, 1, 1)

                        button = Gtk.Button(label="Z-Normal FB")
                        button.connect("clicked", self.on_button_znormal)
                        grid_chv5c.attach(button, 18, 9, 1, 1)

                        button = Gtk.Button(label="STM->HR-AFM Z-Set")
                        button.connect("clicked", self.on_button_toAFM)
                        grid_chv5c.attach(button, 17, 10, 1, 1)
                        button = Gtk.Button(label="HRAFM->STM Z-FB")
                        button.connect("clicked", self.on_button_toSTM)
