...
extern spm_template_hwi_dev *spm_template_hwi; // instance of the HwI derived XSM_Hardware class
...
int SPM_Template_Control::Probing_event_setup_scan (int ch, 
...
int SPM_Template_Control::Probing_eventcheck_callback( GtkWidget *widget, SPM_Template_Control *dspc){
	...
	GArray **garr_hdr;
        int xip=-1, yip=-1;
	XSM_DEBUG_PG ("SPM_Template_Control::Probing_eventcheck_callback -- enter");
	...
	if (popped > 0)
        ...
        XSM_DEBUG_PG("DBG-M5");
	XSM_DEBUG_PG ("SPM_Template_Control::Probing_eventcheck_callback -- exit");
	return 0;
}

void SPM_Template_Control::probedata_visualize (GArray *probedata_x, GArray *probedata_y, GArray *probedata_sec,
...
//			pc_av->show ();
	...
	delete UXaxis;
	delete UYaxis;
	XSM_DEBUG_PG ("SPM_Template_Control::probedata_visualize -- exit");
}


int SPM_Template_Control::Probing_graph_callback( GtkWidget *widget, SPM_Template_Control *dspc, int finish_flag){
// show and update data pv graph
	...
	XSM_DEBUG_PG ("SPM_Template_Control::Probing_graph_callback -- enter");
...
// on-the-fly visualisation graphs update
	...
	XSM_DEBUG_PG ("SPM_Template_Control::Probing_graph_callback -- done");
	return 0;
}

// abort probe and stop fifo read, plot data until then
int SPM_Template_Control::Probing_abort_callback( GtkWidget *widget, SPM_Template_Control *dspc){
...
int SPM_Template_Control::Probing_save_callback( GtkWidget *widget, SPM_Template_Control *dspc){
	int sec, bi;
	double t0=0.;
...
void SPM_Template_Control::push_probedata_arrays (){
...
GArray** SPM_Template_Control::pop_probedata_arrays (){
	pv_lock = TRUE;
	if (probedata_list){
...
GArray** SPM_Template_Control::pop_probehdr_arrays (){
	pv_lock = TRUE;
	if (probehdr_list){
...
void SPM_Template_Control::free_probedata_array_set (GArray** garr, SPM_Template_Control *dc){
        if (!garr) return;
	dc->pv_lock = TRUE;
	for (int i=0; i<NUM_PROBEDATA_ARRAYS; ++i)
...
void SPM_Template_Control::free_probehdr_array_set (GArray** garr, SPM_Template_Control *dc){
        if (!garr) return;
	dc->pv_lock = TRUE;
 	for (int i=0; i<NUM_PROBEDATA_ARRAYS; ++i)
                g_array_unref (garr[i]);
...
void SPM_Template_Control::free_probedata_arrays (){
	pv_lock = TRUE;
	if (probedata_list){
                g_slist_foreach (probedata_list, (GFunc) SPM_Template_Control::free_probedata_array_set, this);
                g_slist_free (probedata_list);
                probedata_list = NULL;	
        }

	if (probehdr_list){
                g_slist_foreach (probehdr_list, (GFunc) SPM_Template_Control::free_probehdr_array_set, this);
...
void SPM_Template_Control::init_probedata_arrays (){
	pv_lock = TRUE;
	for(int i=0; i<4; ++i)
...
void SPM_Template_Control::add_probevector(){ 
	...
	for (i = PROBEDATA_ARRAY_TIME, j=0; i < PROBEDATA_ARRAY_SEC; ++i, ++j){
		...
		case PROBEDATA_ARRAY_X0:
			val += program_vector_list[sec].f_dx0*multi*fixptm;
			break;
		...
		case PROBEDATA_ARRAY_YS:
			val -= program_vector_list[sec].f_dy*multi*fixptm;
			break;
		case PROBEDATA_ARRAY_ZS:
			val -= program_vector_list[sec].f_dz*multi*fixptm;
			break;
		case PROBEDATA_ARRAY_U:
			val += program_vector_list[sec].f_du*multi*fixptm;
			break;
		default:
			break; // error!!!!
		}
...
void SPM_Template_Control::add_probe_hdr(double pv[9]){ 
	int i,j;
