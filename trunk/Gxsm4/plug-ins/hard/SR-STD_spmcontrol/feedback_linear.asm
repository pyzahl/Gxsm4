...
;____________________________________________________________________________
	...
	.asg *(_iobuf+5), fb_signal
	.asg *(_iobuf+13), z_scan ; AIC1 later [+13]!!!
;	.asg *(_iobuf+9), i_abs_test	; AIC2 testing only
;	.asg *(_iobuf+10), i_log_test	; AIC3 testing only
;	.asg *(_iobuf+11), i_delta_test	; AIC4 testing only
;	.asg *(_iobuf+14), i_setpoint_test	; AIC5 testing only
...
;____________________________________________________________________________
	...
	.global	_asm_feedback_lin
	.global	_asm_calc_setpoint_lin
...
_asm_feedback_lin:
		...
		STH		A, fb_ist		 ; linear feedback, no transformation here!!
		SFTA 	A, -1

		;; compute Proportional-Integral-Feedback signal, SATuration A:
		;; delta = ist-soll
		;; i_pl_sum += CI*delta -- includes external plane summing into
		;; z = CP*delta + i_pl_sum 
	...
	;; 
	;; recalculate "linear setpoint" from voltage - no transformation
	;; ____________________________________________________________

_asm_calc_setpoint_lin:
