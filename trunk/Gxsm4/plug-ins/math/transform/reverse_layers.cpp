 ...
 * 
 * Gxsm Plugin Name: reverse_layers.C
 * ========================================
 * 
...
% PlugInDocuCaption: Multi Dimensional Layer Reverse
% PlugInName: reverse_layers
% PlugInAuthor: Percy Zahl
% PlugInAuthorEmail: zahl@users.sf.net
% PlugInMenuPath: math-transformations-sectionReverse Layers
...
// Plugin Prototypes
static void reverse_layers_init( void );
static void reverse_layers_about( void );
static void reverse_layers_configure( void );
static void reverse_layers_cleanup( void );
...
// "OneSrc" Prototype
 static gboolean reverse_layers_run( Scan *Src, Scan *Dest );
#else
// "TwoSrc" Prototype
 static gboolean reverse_layers_run( Scan *Src1, Scan *Src2, Scan *Dest );
#endif

// Fill in the GxsmPlugin Description here
GxsmPlugin reverse_layers_pi = {
  ...
  g_strdup("Multi dimensional layer reverse"),                   
  // Author(s)
  "Percy Zahl",
  ...
  N_("Multi dimensional layer reverse"),
  // more info...
  "Multi dimensional layer reverse",
  ...
  reverse_layers_about,
  // configure-function, can be "NULL"
  // can be called by "Plugin Details"
  reverse_layers_configure,
  ...
  reverse_layers_cleanup
};

// special math Plugin-Strucure, use
// GxsmMathOneSrcPlugin reverse_layers_m1s_pi -> "OneSrcMath"
// GxsmMathTwoSrcPlugin reverse_layers_m2s_pi -> "TwoSrcMath"
#ifdef GXSM_ONE_SRC_PLUGIN__DEF
 GxsmMathOneSrcPlugin reverse_layers_m1s_pi
#else
 GxsmMathTwoSrcPlugin reverse_layers_m2s_pi
#endif
 = {
...
static const char *about_text = N_("Gxsm reverse_layers Plugin\n\n"
                                   "Multi dimensional transposition");

// Symbol "get_gxsm_plugin_info" is resolved by dlsym from Gxsm, used to get Plugin's info!! 
// Essential Plugin Function!!
GxsmPlugin *get_gxsm_plugin_info ( void ){ 
  reverse_layers_pi.description = g_strdup_printf(N_("Gxsm MathOneArg reverse_layers plugin %s"), VERSION);
  return &reverse_layers_pi; 
}
...
GxsmMathOneSrcPlugin *get_gxsm_math_one_src_plugin_info( void ) {
  return &reverse_layers_m1s_pi; 
}
#else
GxsmMathTwoSrcPlugin *get_gxsm_math_two_src_plugin_info( void ) { 
  return &reverse_layers_m2s_pi; 
}
#endif
...
static void reverse_layers_about(void)
{
  const gchar *authors[] = { reverse_layers_pi.authors, NULL};
  gtk_show_about_dialog (NULL, 
			 "program-name",  reverse_layers_pi.name,
			 "version", VERSION,
...
static void reverse_layers_configure(void)
{
  if(reverse_layers_pi.app)
    reverse_layers_pi.app->message("reverse_layers Plugin Configuration");
}
...
// run-Function
 static gboolean reverse_layers_run(Scan *Src, Scan *Dest)
{
	main_get_gapp()->progress_info_new ("Reversing Layers", 2); // setup an progress indicator
	...
	for (int time_index=0; time_index < n_times; ++time_index){
		...
		for (int layer_index=0; layer_index < n_layers; ++layer_index){
			...
			Dest->mem2d->SetLayer (n_layers-layer_index-1);
			std::cout << "CopyFrom..." << std::endl;
			Dest->mem2d->CopyFrom (m, 0,0, 0,0, Src->mem2d->GetNx (), Src->mem2d->GetNy ());
			std::cout << "SetVLookup..." << std::endl;
			Dest->mem2d->data->SetVLookup (n_layers-layer_index-1, m->data->GetVLookup (layer_index));
			...
			if (m->layer_information_list[layer_index]){
				...
				for (int j = 0; j<nj; ++j){
					...
					if (li)
						Dest->mem2d->add_layer_information (n_layers-layer_index-1, new LayerInformation (li));
				}
			}
		...
		Dest->append_current_to_time_elements (time_index, frame_time);
		std::cout << "OK." << std::endl;
	}
